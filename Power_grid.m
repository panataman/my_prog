classdef Power_grid < handle % it have to be a handle class because of call by reference (speed)
    ...
    % invoke constructor class Power_grid
        function obj = Power_grid(configuration_grid, number_steps, days)
        % in configuration_grid all data about which how many supermarkets are connected on which bus
	% invoce constructor class Bus and save into properties busses of object Power_grid
            for i = 1 : length([configuration_grid{:,1}])
                obj.busses = [obj.busses Bus({configuration_grid{i,2}}, i, number_steps, days)];
            end
